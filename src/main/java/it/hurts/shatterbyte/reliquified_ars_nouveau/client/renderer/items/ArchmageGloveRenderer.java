...
public class ArchmageGloveRenderer implements IRelicRenderer {
    private final ArchmageGloveSlimModel slimModel;
    private final ArchmageGloveWideModel wideModel;

    public ArchmageGloveRenderer() {
        this.slimModel = new ArchmageGloveSlimModel(Minecraft.getInstance().getEntityModels().bakeLayer(ArchmageGloveSlimModel.LAYER));
        this.wideModel = new ArchmageGloveWideModel(Minecraft.getInstance().getEntityModels().bakeLayer(ArchmageGloveWideModel.LAYER));
    }
    ...
    public <E extends LivingEntity, EM extends EntityModel<E>> void render(ItemStack stack, SlotContext slotContext, PoseStack poseStack, RenderLayerParent<E, EM> parent, MultiBufferSource bufferSource, int light, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        var relic = (ArchmageGloveItem) stack.getItem();
        ...
        model.renderToBuffer(poseStack, bufferSource.getBuffer(RenderType.entityCutout(ResourceLocation.fromNamespaceAndPath(ReliquifiedArsNouveau.MODID, "textures/item/model/archmage_glove_" + (isSlim ? "slim" : "wide") + ".png"))), relic.getRelicData(entity, stack).isFlawless() ? LightTexture.FULL_BRIGHT : light, OverlayTexture.NO_OVERLAY);

        model.renderToBuffer(poseStack, bufferSource.getBuffer(RenderType.entityCutout(ResourceLocation.fromNamespaceAndPath(ReliquifiedArsNouveau.MODID, "textures/item/model/archmage_glove_gem.png"))), LightTexture.FULL_BRIGHT, OverlayTexture.NO_OVERLAY);
